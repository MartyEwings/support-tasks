...
case $command in
  ...
  manual_gitlab_webhook_hit)
	...
	curl -X GET \
  	...
  	https://"${CONSOLE}":4433/status/v1/services | python -m json.tool || fail "unable to get service status"
	  ;;
esac
else
  fail  "Not a Puppet master node, exiting"
